...
const schema = require('../../utils/schemas').stm_details_schema;

stm_details = (request, response, next) => {
    ...
    validator(request.params, schema, (error, value) => {
        if (error) {
            ...
            stmModel.findAll({
                where: { stm_session_id: request.params.s_id },
                include: [{ model: userDetailsModel, as: 'reviewer', attributes: [[userDetailsModel.sequelize.literal("reviewer.first_name || ' ' || reviewer.last_name"), 'full_name']] },
                { model: userDetailsModel, as: 'reviewee', attributes: [[userDetailsModel.sequelize.literal("reviewee.first_name || ' ' || reviewee.last_name"), 'full_name']] },
                { model: templateModel, as: 'templates', attributes: ['t_name'] }]
            })
                .then((result) => {
                    ...
                    for (var i in result) {
                        for (let j in result[i]) {
                            menu.push({ 'reviewer_name': i, 'template_name': j, 'reviewees': result[i][j] });
                        }
                    }
